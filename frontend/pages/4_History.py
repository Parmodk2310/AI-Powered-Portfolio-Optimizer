...
def _format_run_time_ist(value) -> str:
    ...
    return timestamp.tz_convert(IST_TIMEZONE).strftime("%Y-%m-%d %H:%M:%S IST")
...
if latest:
    ...
    metrics = [
        {
            "label": "Sharpe Ratio",
            "value": f"{latest.get('sharpe_ratio', 0):.3f}",
            "tone": "cyan",
            "icon": "◉",
        },
        {
            "label": "Expected Return",
            "value": f"{latest.get('expected_return', 0)*100:.2f}%",
            "tone": "positive" if latest.get("expected_return", 0) > 0 else "negative",
            "icon": "▲",
        },
        {
            "label": "Volatility",
            "value": f"{latest.get('volatility', 0)*100:.2f}%",
            "tone": "negative",
            "icon": "◊",
        },
        {
            "label": "Alpha Used",
            "value": f"{latest.get('alpha_used', 0):.2f}",
            "tone": "accent",
            "icon": "◈",
        },
    ]
...
comparable_runs = [run for run in history if _ticker_universe(run) == latest_universe]
...
trend_df["date"] = pd.to_datetime(
    trend_df["date"], errors="coerce", utc=True
...
trend_df = trend_df.groupby("run_day", as_index=False).tail(1)
...
def _normalize_recommendations(items):
    ...
    for item in items or []:
        if isinstance(item, dict):
            ticker = (
                item.get("ticker") or item.get("symbol") or item.get("name") or "N/A"
            )
            text = (
...
if len(history) >= 2:
    ...
    for ticker in sorted(set(latest_recs) | set(previous_recs)):
        ...
        badge_html = (
            badge("UPDATED", "warning") if changed else badge("UNCHANGED", "positive")
        )
...
# ── All Past Runs Table ─────────────────────────────────────
section_header(
    "All Past Runs", f"{len(history)} records · Times shown in IST", accent="green"
)
    ...
    rows.append(
        {
            ...
            "Sharpe": (
                f"{run['sharpe_ratio']:.4f}" if run.get("sharpe_ratio") else "N/A"
            ),
            "Return": (
                f"{run['expected_return']*100:.2f}%"
                if run.get("expected_return")
                else "N/A"
            ),
            "Volatility": (
                f"{run['volatility']*100:.2f}%" if run.get("volatility") else "N/A"
            ),
        }
    )
