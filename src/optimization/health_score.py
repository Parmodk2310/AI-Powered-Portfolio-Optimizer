...
def calculate_health_score(
    *,
    ...
    high_corr_pairs = (
        risk_report.get("correlation", {}).get("high_correlation_pairs", []) or []
    )
    ...
    if news_counts is None:
        sent_vals = [
            float(value) for value in sentiment_scores.values() if value is not None
        ]
    else:
        sent_vals = [
    ...
    consistency_factor = _piecewise(
        dispersion, [(0.0, 1.0), (0.20, 0.98), (0.40, 0.92), (0.70, 0.82), (1.0, 0.75)]
    )
    sentiment_score = (
        directional * (0.85 + 0.15 * _clip(coverage, 0, 1)) * consistency_factor
    )
    ...
    risk_completeness = sum(1 for k in completeness_keys if risk_report.get(k)) / len(
        completeness_keys
    )
    news_depth = 1.0
    if news_counts is not None and n:
        depths = [
            min(max(int(news_counts.get(t, 0)), 0), 10) / 10.0 for t in final_weights
        ]
        news_depth = sum(depths) / len(depths) if depths else 0.0
    data_quality_score = 100.0 * (
        0.40 * _clip(coverage, 0, 1) + 0.40 * risk_completeness + 0.20 * news_depth
    )
    ...
    if candidate_scores and len(candidate_scores) > 1:
        ...
        spread = (
            sum((x - cmean) ** 2 for x in candidate_scores) / len(candidate_scores)
        ) ** 0.5
        stability_score = _piecewise(
            spread, [(0, 100), (1.5, 96), (3, 90), (5, 80), (8, 65), (12, 45), (20, 20)]
        )
    else:
        spread = None
    ...
    if max_weight > 0.40:
        ...
        penalties.append(
            {"name": "single_position_concentration", "points": round(p, 2)}
        )
        penalty_total += p
    ...
    if max_weight > 0.30:
        improvements.append(
            f"Reduce the largest position from {max_weight*100:.1f}% toward 25-30%."
        )
    if top2 > 0.55:
        improvements.append(
            f"Reduce top-two exposure from {top2*100:.1f}% toward 45-55%."
        )
    if dd > 12:
        improvements.append(
            f"Improve drawdown control; current max drawdown is {dd:.1f}%."
        )
    if volatility > 0.18:
        improvements.append(
            f"Lower annualized volatility from {volatility*100:.1f}% toward 15-18%."
        )
    if avg_sent < 0:
        improvements.append(
            f"News sentiment is slightly negative ({avg_sent:+.3f}); avoid overweighting weak-sentiment names."
        )
    ...
    return {
        ...
        "diagnostics": {
            ...
            "expected_shortfall_95": (
                None
                if expected_shortfall95 is None
                else round(float(expected_shortfall95), 6)
            ),
            "candidate_score_spread": None if spread is None else round(spread, 4),
            "sharpe_gain_vs_equal_weight": (
                None if sharpe_gain is None else round(sharpe_gain, 6)
            ),
        },
