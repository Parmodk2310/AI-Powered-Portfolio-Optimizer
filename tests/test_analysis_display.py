...
def test_adaptive_search_does_not_scale_health_or_sharpe():
    source = Path("frontend/pages/3_Analysis.py").read_text(encoding="utf-8")
...
def test_performance_tab_uses_independent_figure():
    ...
    assert "ticker_return_fig = go.Figure()" in (performance_source)
    assert 'key="ticker_cumulative_returns"' in performance_source
