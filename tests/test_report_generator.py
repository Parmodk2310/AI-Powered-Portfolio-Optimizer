...
def test_ai_markdown_is_rendered():
    rendered = render_ai_commentary("**Evidence quality:** Limited")
...
def test_ai_script_is_removed():
    rendered = render_ai_commentary("<script>alert('unsafe')</script>Safe text")
...
def test_report_source_contains_complete_news_pipeline():
    source = Path("frontend/pages/report_generator.py").read_text(encoding="utf-8")
...
def test_live_and_report_consume_canonical_rebalance_plan():
    analysis_source = Path("frontend/pages/3_Analysis.py").read_text(encoding="utf-8")

    report_source = Path("frontend/pages/report_generator.py").read_text(
        encoding="utf-8"
    )

    expected_lookup = "plan_entry = rebalance_plan.get(ticker, {})"

    assert expected_lookup in analysis_source
    assert expected_lookup in report_source

    assert 'rebalance_plan = results.get("rebalance_plan", {})' in analysis_source
    assert 'rebalance_plan = results.get("rebalance_plan", {})' in report_source
